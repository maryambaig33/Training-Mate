      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {SCHEDULE.map((session) => (
            <div key={session.id} className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 hover:bg-gray-800 border border-gray-700 hover:border-mate-orange transition-all duration-300 group shadow-lg">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex bg-gray-900/50 rounded-lg px-2 py-1">
                  {[...Array(3)].map((_, i) => (
                    <svg 
                      key={i}
                      className={`w-4 h-4 ${i < session.intensity ? 'text-orange-500' : 'text-gray-700'}`} 
                      fill="currentColor" 
                      viewBox="0 0 20 20"
                    >
              ...
              <div className="flex items-center justify-between border-t border-gray-700 pt-4 mt-auto">
                ...
                <button className="bg-white text-mate-blue hover:bg-mate-orange hover:text-white text-xs font-bold px-4 py-2 rounded-full transition-colors uppercase tracking-wide">
                    Book
                </button>
              </div>
            </div>
        ...
        <div className="mt-16 text-center">
            ...
            <a href="#ai-coach" className="inline-flex items-center text-mate-orange font-bold hover:text-white transition-colors border-b-2 border-mate-orange hover:border-white pb-1">
                Ask Coach Mate for a recommendation &rarr;
            </a>
        </div>
      </div>
    </section>
